...
static struct kmem_cache *m_cache;
...
static int identity_create(char *name, int id)
{
	...
	elem = kmem_cache_alloc(m_cache, GFP_KERNEL);
	if (!elem)
		return -ENOMEM;
...
void identity_destroy(int id)
{
	struct identity *id_elem = NULL;
        ...
        list_for_each_entry(id_elem, &mylinkedlist, mylist)
        {
			...
			kmem_cache_free(m_cache, id_elem);
		}       
        }
}
...
static int __init my_init(void)
{
	...
	m_cache = kmem_cache_create("identities_cache",
		       sizeof(struct identity), sizeof(struct identity),
		       0, NULL);
	if (!m_cache) {
		pr_alert("[CACHE ERROR] Error while creating cache\n");
		return -ENOMEM;
	}
